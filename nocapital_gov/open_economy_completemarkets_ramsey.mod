...
var lmult1,  lmult2,  lmult3,  lmult4,  lmult5,  lmult6,  lmult7,   
    ...
    lmult29,  lmult30,  lmult31,  lmult32; 
...
lmult24 - lmult22 - cc_m^(1/(rho_cc_p + 1) - 1)*lmult25*(cc_d^(1/(rho_cc_p + 1))*omega_cc_p^(rho_cc_p/(rho_cc_p + 1)) + cc_m^(1/(rho_cc_p + 1))*(1 - omega_cc_p)^(rho_cc_p/(rho_cc_p + 1)))^rho_cc_p*(1 - omega_cc_p)^(rho_cc_p/(rho_cc_p + 1)); 
lmult27 - lmult21 - cc_m2^(1/(rho_cc_p + 1) - 1)*lmult28*(cc_d2^(1/(rho_cc_p + 1))*omega_cc_p^(rho_cc_p/(rho_cc_p + 1)) + cc_m2^(1/(rho_cc_p + 1))*(1 - omega_cc_p)^(rho_cc_p/(rho_cc_p + 1)))^rho_cc_p*(1 - omega_cc_p)^(rho_cc_p/(rho_cc_p + 1)); 
...
lmult19 + lmult21 - lmult7*((phi - 1)*((tau_p + 1)/theta_p - (sigma*(theta_p + 1))/theta_p) - dphidp*(tau_p - sigma + 1)) + (dphidlp*lmult7(-1)*(tau_p - sigma + 1))/(nbeta*(rate1(-1) + 1)); 
...
lmult11*((infl^2*phi_p)/infl_ss^2 + (2*infl*phi_p*(infl/infl_ss - 1))/infl_ss) - lmult10*((infl*phi_p)/infl_ss^2 + (phi_p*(infl/infl_ss - 1))/infl_ss) - (lmult9*phi_p*(infl/infl_ss - 1))/infl_ss - (beta_p*lmult5(-1)*r_price_cc(-1)*(cc(-1) - muc(-1))^gamma_p)/(infl^2*nbeta*r_price_cc*(cc - muc)^gamma_p); 
lmult14*((infl2^2*phi2_p)/infl_ss^2 + (2*infl2*phi2_p*(infl2/infl_ss - 1))/infl_ss) - lmult13*((infl2*phi2_p)/infl_ss^2 + (phi2_p*(infl2/infl_ss - 1))/infl_ss) - (lmult12*phi2_p*(infl2/infl_ss - 1))/infl_ss - (beta_p*lmult6(-1)*r_price_cc2(-1)*(cc2(-1) - muc2(-1))^gamma_p)/(infl2^2*nbeta*r_price_cc2*(cc2 - muc2)^gamma_p); 
lmult15 - lmult19*exp(theta) + chi0_p*labor^chi_p*lmult1 - lmult31*tau_l*wages; 
lmult16 - lmult20*exp(theta2) + chi0_p*labor2^chi_p*lmult3 - lmult32*tau_l2*wages2; 
lmult9 - gdp*lmult7*((tau_p + 1)/theta_p - (sigma*(theta_p + 1))/theta_p); 
lmult12 - gdp2*lmult8*((tau_p + 1)/theta_p - (sigma2*(theta_p + 1))/theta_p); 
lmult5/(rate1 + 1)^2 - (dphidlp(+1)*gdp(+1)*lmult7*(tau_p - sigma(+1) + 1))/(rate1 + 1)^2; 
lmult6/(rate2 + 1)^2 - (dphidlp2(+1)*gdp2(+1)*lmult8*(tau_p - sigma2(+1) + 1))/(rate2 + 1)^2; 
lmult29 + (lmult30*r_price_cc2)/r_price_cc - (cc*lmult24*r_price_cc*(r_price_cc/r_exchg)^((rho_cc_p + 1)/rho_cc_p - 1)*(omega_cc_p - 1)*(rho_cc_p + 1))/(r_exchg^2*rho_cc_p); 
lmult29/r_exchg2^2 - (cc2*lmult27*r_price_cc2*(r_price_cc2/r_exchg2)^((rho_cc_p + 1)/rho_cc_p - 1)*(omega_cc_p - 1)*(rho_cc_p + 1))/(r_exchg2^2*rho_cc_p); 
(beta_p*lmult5*(cc - muc)^gamma_p)/(infl(+1)*r_price_cc(+1)*(cc(+1) - muc(+1))^gamma_p) - (cc*lmult23*omega_cc_p*r_price_cc^((rho_cc_p + 1)/rho_cc_p - 1)*(rho_cc_p + 1))/rho_cc_p - (lmult30*r_exchg*r_price_cc2)/r_price_cc^2 + (cc*lmult24*(r_price_cc/r_exchg)^((rho_cc_p + 1)/rho_cc_p - 1)*(omega_cc_p - 1)*(rho_cc_p + 1))/(r_exchg*rho_cc_p) - (beta_p*lmult5(-1)*r_price_cc(-1)*(cc(-1) - muc(-1))^gamma_p)/(infl*nbeta*r_price_cc^2*(cc - muc)^gamma_p) - (lmult15*wages*(tau_l - 1)*(-(wages*(tau_l - 1))/(chi0_p*r_price_cc*(cc - muc)^gamma_p))^(1/chi_p - 1))/(chi_p*chi0_p*r_price_cc^2*(cc - muc)^gamma_p); 
(lmult30*r_exchg)/r_price_cc - (cc2*lmult26*omega_cc_p*r_price_cc2^((rho_cc_p + 1)/rho_cc_p - 1)*(rho_cc_p + 1))/rho_cc_p + (beta_p*lmult6*(cc2 - muc2)^gamma_p)/(infl2(+1)*r_price_cc2(+1)*(cc2(+1) - muc2(+1))^gamma_p) + (cc2*lmult27*(r_price_cc2/r_exchg2)^((rho_cc_p + 1)/rho_cc_p - 1)*(omega_cc_p - 1)*(rho_cc_p + 1))/(r_exchg2*rho_cc_p) - (beta_p*lmult6(-1)*r_price_cc2(-1)*(cc2(-1) - muc2(-1))^gamma_p)/(infl2*nbeta*r_price_cc2^2*(cc2 - muc2)^gamma_p) - (lmult16*wages2*(tau_l2 - 1)*(-(wages2*(tau_l2 - 1))/(chi0_p*r_price_cc2*(cc2 - muc2)^gamma_p))^(1/chi_p - 1))/(chi_p*chi0_p*r_price_cc2^2*(cc2 - muc2)^gamma_p); 
...
(lmult15*wages*(-(wages*(tau_l - 1))/(chi0_p*r_price_cc*(cc - muc)^gamma_p))^(1/chi_p - 1))/(chi_p*chi0_p*r_price_cc*(cc - muc)^gamma_p) - labor*lmult31*wages; 
(lmult16*wages2*(-(wages2*(tau_l2 - 1))/(chi0_p*r_price_cc2*(cc2 - muc2)^gamma_p))^(1/chi_p - 1))/(chi_p*chi0_p*r_price_cc2*(cc2 - muc2)^gamma_p) - labor2*lmult32*wages2; 
lmult1 - lmult2 + omega_welf1; 
lmult3 - lmult4 + omega_welf2; 
(lmult15*(tau_l - 1)*(-(wages*(tau_l - 1))/(chi0_p*r_price_cc*(cc - muc)^gamma_p))^(1/chi_p - 1))/(chi_p*chi0_p*r_price_cc*(cc - muc)^gamma_p) - labor*lmult31*tau_l - lmult17*exp(-theta); 
(lmult16*(tau_l2 - 1)*(-(wages2*(tau_l2 - 1))/(chi0_p*r_price_cc2*(cc2 - muc2)^gamma_p))^(1/chi_p - 1))/(chi_p*chi0_p*r_price_cc2*(cc2 - muc2)^gamma_p) - labor2*lmult32*tau_l2 - lmult18*exp(-theta2); 
lmult2 - lmult2(-1); 
lmult4 - lmult4(-1); 
