...
var cc, rs1, infl, sigma, gdp, 
    phi, dphidp, dphidlp,
    labor, wages, Util1, 
    //Welf1, 
    cc_d, cc_m, r_price_cc, r_exchg, nfa, 
    cc2, rs2, infl2, sigma2, gdp2,
    phi2, dphidp2, dphidlp2,
    labor2, wages2, Util2, 
    //Welf2, 
    cc_d2, cc_m2, r_price_cc2, r_exchg2,
...
rate1 = rs1;
rs1 =  (- 1 + 1/ beta_p ) 
	      +  rs_rsl_p *(rs1(-1) - (- 1 + 1/beta_p ) ) 
	      +  (1-rs_rsl_p)*rs_dp_p * (infl - infl_ss)
	      +  (1-rs_rsl_p)*rs_gdp_p * (log(gdp)- log(gdp(-1)));
...
rate2 = rs2;
rs2 =  (- 1 + 1/ beta_p ) 
	      +  rs_rsl_p *(rs2(-1) - (- 1 + 1/beta_p ) ) 
	      +  (1-rs_rsl_p)*rs_dp_p * (infl2 - infl_ss)
	      +  (1-rs_rsl_p)*rs_gdp_p * (log(gdp2)- log(gdp2(-1)));

//consumption Euler equations
beta_p*(r_price_cc/r_price_cc(1)/infl(1))*(cc(1) - muc(1))^(-gamma_p)/(cc-muc)^(-gamma_p) = 1/(1 + rs1);
beta_p*(r_price_cc2/r_price_cc2(1)/infl2(1))*(cc2(1) - muc2(1))^(-gamma_p)/(cc2-muc2)^(-gamma_p) = 1/(1+rs2);

//price setting
0 = (-(1+tau_p)/theta_p + sigma*(1 + theta_p)/theta_p)*(1-phi)*gdp
    - ((1 + tau_p) - sigma)*gdp*dphidp
    - (1/(1+rs1))*((1+tau_p) - sigma(1))*gdp(1)*dphidlp(1);

0 = (-(1+tau_p)/theta_p + sigma2*(1 + theta_p)/theta_p)*(1-phi2)*gdp2
    - ((1 + tau_p) - sigma2)*gdp2*dphidp2
    - (1/(1+rs2))*((1+tau_p) - sigma2(1))*gdp2(1)*dphidlp2(1);
...
1/(phi_nfa*(1+rs2)) =  infl(1)/(1+rs1)*r_exchg(1)/r_exchg/infl2(1);
...
1/(phi_nfa*(1+rs2))*nfa-r_exchg/r_exchg(-1)/infl2*nfa(-1)*gdp(-1)/gdp = (cc_m2-r_exchg*cc_m)/gdp;
