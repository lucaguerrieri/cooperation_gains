...
%
% - The foreign interest rate is seen as an exogenous variable by the home
%   country.
...
% (except for policy rule when ramsey_flag = 1)
      ...
      if ramsey_flag & (ruleqflag > 0),
        rule_string = [lhs_string,rhs_string,';'];
        if strfind(rule_string,welf_vname_other),
            error('Must set welfare as only variable on LHS of equation');
        end;
        ruleqmat = strvcat(ruleqmat,rule_string);
        ruleqflag = 0;
        rulestopline = [rulestopline, iline];
...
if ramsey_flag,
  ...
  if ~length(iwelf),
    error('Welfare for current country not declared in endogenous variable list');
  end;
  ...
  if delete_rate_count == 0 | delete_welf_count == 0,
...
if ramsey_flag,
  eqmat = strvcat(eqmat,Welf_string);
end;
...
% Set up policymaker's first-order conditions
  ...
  lmss_coefdefs{1} = ['c',num2str(c),'lmss_mat = zeros(',num2str(nendog),',',num2str(nbehaveqs),');'];
  lmss_coefdefs{2} = ['c',num2str(c),'lmss_vec = zeros(',num2str(nendog),',1);'];
  icoefdef = 2;
...
% Start by differentiating period utility with respect to the variable
  ...
  icoefdef = icoefdef + 1;
  lmss_coefdefs{icoefdef} = ['c',num2str(c),'lmss_vec(',num2str(iev),') = ',char(tmpval),';']; %originally char(struct2cell(tmpval))
...
% Now differentiate each behavioral equation.
  ...
  eval(['tmpval = sym(0) + ',lmss_string,';']);
  icoefdef = icoefdef + 1;
  lmss_coefdefs{icoefdef} = ['c',num2str(c),'lmss_mat(',num2str(iev),',',num2str(jbeq),') = ',char(tmpval),';']; %originally char(struct2cell(tmpval))
  end;
...
lmss_coefdefs = vertcat(lmss_coefdefs,' ');
...
if c == 1,   
 ...
 c1focmat = focmat;
 c1lmss_coefdefs = lmss_coefdefs;
else,
 c2lmultlist = lmultlist;
 c2lmss_list = lmss_list;
 c2focmat = focmat;
 c2lmss_coefdefs = lmss_coefdefs;
end;
...
lmss_coefdefs = vertcat(c1lmss_coefdefs,c2lmss_coefdefs);
