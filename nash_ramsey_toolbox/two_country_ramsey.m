...
instr_vname_current = cat(2,'infl',num2str(c));
util_vname_current = cat(2,'Util',num2str(c));
welf_vname_current = cat(2,'Welf',num2str(c));

instr_vname_other = cat(2,'infl',num2str(o));
...
shkmat = '';
endovarmat = '';
exovarmat = instr_vname_other; % The current country sees the other country's interest
...
% foreign interest rate, utility, or welfare.
  ...
  elseif varflag | strncmpi(rawline,'var ',4),
    ...
    if strcmp(varname, instr_vname_other),
...
if ramsey_flag,
  ...
  irate_other = strmatch(instr_vname_other, endovarmat, 'exact');
